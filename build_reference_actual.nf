...
workflow{
    ...
    def denylist_path = "${params.actual_data_dir}/reference/denylist.bed"
    def genome_gtf_path = "${params.actual_data_dir}/reference/annotations.gtf"
	def fasta_index_path = "${params.actual_data_dir}/genome.fa.fai"
    def genome_dict_path = "${params.actual_data_dir}/genome.dict"
    def star_index_path = "${params.actual_data_dir}/STAR_index"
    def prepared_vcf_path = "${params.actual_data_dir}/merged.filtered.recode.vcf.gz"
	
     // Load and parse sample metadata
    def samples_channel = Channel.fromPath(params.actual_csv_file)
    ...
    if (params.only_fastqc_fastp) {
		...
		def snpeff_jar_path = "${params.actual_data_dir}/snpEff/snpEff.jar"
		def snpeff_config_path = "${params.actual_data_dir}/snpEff/snpEff.config"
		...
		def snpeff_db_path = "${params.actual_data_dir}/snpEff/snpEff/data/${params.genomedb}"
		...
		def arriba_dir_path = "${params.actual_data_dir}/ARRIBA/arriba_v2.4.0"
		def arriba_tool = file(arriba_dir_path).exists() ? 
			file(arriba_dir_path) : 
			DOWNLOAD_ARRIBA()

		log.info "Checking VEP cache existence..."
		def vep_cache_path = "${params.actual_data_dir}/vep_cache/homo_sapiens/110_GRCh38"
		def vep_cache = file(vep_cache_path).exists() ? 
			file(vep_cache_path) : 
			DOWNLOAD_VEP_CACHE()

		log.info "Checking ClinVar files existence..."
		def clinvar_vcf_path = "${params.actual_data_dir}/VEP/clinvar.vcf.gz"
		def clinvar_tbi_path = "${params.actual_data_dir}/VEP/clinvar.vcf.gz.tbi"
