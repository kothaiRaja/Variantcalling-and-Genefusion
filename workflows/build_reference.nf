...
include { CHECK_OR_DOWNLOAD_REF_GENOME } from '../modules/references/reference_genome.nf'
include { GUNZIP } from '../modules/references/gunzip.nf'
include { DOWNLOAD_GENOME_INDEX } from '../modules/references/genome_index.nf'
...
include { CHECK_OR_DOWNLOAD_GTF } from '../modules/references/gtf_annotation.nf'
...
genome_ch = 
    ...
    file("${params.ref_base}/reference/genome.fa").exists() ? 
        Channel.fromPath("${params.ref_base}/reference/genome.fa") :
...
genome_ch.view { genome_path ->  
    if (genome_path.toString().contains('/work/')) {
        referenceGenomePath = "${params.ref_base}/reference/genome.fa"
    } else {
...
genome_index_ch = 
    ...
    file("${params.ref_base}/reference/genome.fa.fai").exists() ? 
        Channel.fromPath("${params.ref_base}/reference/genome.fa.fai") :
...
genome_index_ch.view { genome_index_path ->  
    if (genome_index_path.toString().contains('/work/')) {
        genomeIndexPath = "${params.ref_base}/reference/genome.fa.fai"
    } else {
...
genome_dict_ch = 
    ...
    file("${params.ref_base}/reference/genome.dict").exists() ? 
        Channel.fromPath("${params.ref_base}/reference/genome.dict") :
...
genome_dict_ch.view { genome_dict_path ->  
    if (genome_dict_path.toString().contains('/work/')) {
        genomeDictPath = "${params.ref_base}/reference/genome.dict"
    } else {
...
gtf_ch = 
    ...
    file("${params.ref_base}/reference/annotations.gtf").exists() ? 
        Channel.fromPath("${params.ref_base}/reference/annotations.gtf") :
...
gtf_ch.view { gtf_path ->  
    if (gtf_path.toString().contains('/work/')) {
        gtfPath = "${params.ref_base}/reference/annotations.gtf"
    } else {
...
exons_bed_ch = 
    file("${params.ref_base}/reference/exons.bed").exists() ? 
        Channel.fromPath("${params.ref_base}/reference/exons.bed") :
...
exons_bed_ch.view { exons_bed_path ->  
    if (exons_bed_path.toString().contains('/work/')) {
        exonsBedPath = "${params.ref_base}/reference/exons.bed"
    } else {
...
star_index_ch = 
    ...
    file("${params.ref_base}/reference/STAR_index").exists() ? 
        Channel.fromPath("${params.ref_base}/reference/STAR_index") :
...
star_index_ch.view { star_index_path ->  
    if (star_index_path.toString().contains('/work/')) {
        starIndexPath = "${params.ref_base}/reference/STAR_index"
    } else {
...
denylist_ch = 
    ...
    file("${params.ref_base}/reference/denylist.bed").exists() ? 
        Channel.fromPath("${params.ref_base}/reference/denylist.bed") :
...
denylist_ch.view { denylist_path ->  
    if (denylist_path.toString().contains('/work/')) {
        denylistPath = "${params.ref_base}/reference/denylist.bed"
    } else {
...
// ========================== SNP VCF Handling ========================== //
    snp_vcf_ch = 
        ...
        file("${params.ref_base}/reference/variants_snp.vcf.gz").exists() ?
            Channel.of(file("${params.ref_base}/reference/variants_snp.vcf.gz")) :
    ...
    if (snp_vcf_path.toString().contains('/work/')) {
        ...
        snpVcfPath = "${params.ref_base}/reference/variants_snp.vcf.gz"
    } else {
    ...
    file("${params.ref_base}/reference/variants_snp.vcf.gz.tbi").exists() ? 
        Channel.of(file("${params.ref_base}/reference/variants_snp.vcf.gz.tbi")) :
    ...
    if (snp_index_path.toString().contains('/work/')) {
        // Force the path to the published directory
        snpIndexPath = "${params.ref_base}/reference/variants_snp.vcf.gz.tbi"
    } else {
        // If it's already in the publish or server directory, keep the original path
    ...
    indels_vcf_ch = 
        ...
        file("${params.ref_base}/reference/variants_indels.vcf.gz").exists() ?
            Channel.of(file("${params.ref_base}/reference/variants_indels.vcf.gz")) :
            CHECK_OR_DOWNLOAD_VARIANTS_INDELS()
			
	indels_vcf_ch.view { indels_vcf_path ->  
    if (indels_vcf_path.toString().contains('/work/')) {
        // Force the path to the published directory
        indelsVcfPath = "${params.ref_base}/reference/variants_indels.vcf.gz"
    } else {
        // If it's already in the publish or server directory, keep the original path
    ...
    file("${params.ref_base}/reference/variants_indels.vcf.gz.tbi").exists() ? 
        Channel.of(file("${params.ref_base}/reference/variants_indels.vcf.gz.tbi")) :
    ...
    if (indels_index_path.toString().contains('/work/')) {
        ...
        indelsIndexPath = "${params.ref_base}/reference/variants_indels.vcf.gz.tbi"
    } else {
...
// Define channels for merged VCF and index
    ...
    if (file("${params.ref_base}/reference/merged.filtered.recode.vcf.gz").exists() &&
        file("${params.ref_base}/reference/merged.filtered.recode.vcf.gz.tbi").exists()) {

        // If files exist in the publish directory, create channels from them
        merged_vcf_ch = Channel.of(file("${params.ref_base}/reference/merged.filtered.recode.vcf.gz"))
        merged_vcf_index_ch = Channel.of(file("${params.ref_base}/reference/merged.filtered.recode.vcf.gz.tbi"))
...
merged_vcf_ch.view { merged_vcf_path ->
    if (merged_vcf_path.toString().contains('/work/')) {
        // Redirect to the published directory
        mergedVcfPath = "${params.ref_base}/reference/merged.filtered.recode.vcf.gz"
    } else {
        // Keep the original path if it's from the server or already published
...
merged_vcf_index_ch.view { merged_vcf_index_path ->
    if (merged_vcf_index_path.toString().contains('/work/')) {
        // Redirect to the published directory
        mergedVcfIndexPath = "${params.ref_base}/reference/merged.filtered.recode.vcf.gz.tbi"
    } else {
        // Keep the original path if it's from the server or already published
...
// ========================== Java Version Check ========================== //
    ...
    if (file("${params.ref_base}/reference/java_check.log").exists()) {
        ...
        java_check_ch = Channel.of(file("${params.ref_base}/reference/java_check.log"))
    ...
    java_check_ch.view { java_log_path ->
        javaLogPath = file("${params.ref_base}/reference/java_check.log").toString()
	...
	} else if (file("${params.ref_base}/Tools/snpEff/snpEff.jar").exists() && 
			file("${params.ref_base}/Tools/snpEff/snpEff.config").exists()) {
    
		println " SnpEff tool found in the publish directory. Skipping download."

		snpeff_jar_ch = Channel.of(file("${params.ref_base}/Tools/snpEff/snpEff.jar"))
		snpeff_config_ch = Channel.of(file("${params.ref_base}/Tools/snpEff/snpEff.config"))

		snpEffJarPath = "${params.ref_base}/Tools/snpEff/snpEff.config"
		snpEffConfigPath = "${params.ref_base}/Tools/snpEff/snpEff.config"
    ...
    if (snpeff_jar_path.toString().contains('/work/')) {
        snpEffJarPath = "${params.ref_base}/Tools/snpEff/snpEff.jar"
    } else {
...
snpeff_db_ch.view { snpeff_db_path ->  
    if (snpeff_db_path.toString().contains('/work/')) {
        ...
        snpEffDbPath = "${params.ref_base}/Tools/snpEff/snpEff/data/${params.genomedb}"
    } else {
...
} else if (file("${params.ref_base}/Tools/ARRIBA/arriba_v2.4.0").exists()) {
    println " Arriba tool found in the publish directory."
    arriba_dir_ch = Channel.of(file("${params.ref_base}/Tools/ARRIBA/arriba_v2.4.0"))
...
arriba_dir_ch.view { arriba_dir_path ->  
    ...
    if (arriba_dir_path.toString().contains('/work/')) {
        ...
        arribaPath = "${params.ref_base}/Tools/ARRIBA/arriba_v2.4.0"
    } else {
...
} else if (file("${params.ref_base}/Tools/VEP/plugins").exists()) {
    println " VEP plugins found in the publish directory."
    vep_plugins_ch = Channel.of(file("${params.ref_base}/Tools/VEP/plugins"))
...
vep_plugins_ch.view { vep_plugins_path ->  
    if (vep_plugins_path.toString().contains('/work/')) {
        vepPluginsPath = "${params.ref_base}/Tools/VEP/plugins/plugins"
    } else {
    ...
    // Capture outside
def refBasePath = params.ref_base ?: System.getProperty('user.dir') + "/reference"

workflow.onComplete {
    try {
        def configDir = new File("${refBasePath}/reference")

        if (!configDir.exists()) {
            println " Creating reference directory: ${configDir}"
            configDir.mkdirs()
        }
