...
workflow BUILD_STAR_INDEX {
    ...
    if (params.star_genome_index && file(params.star_genome_index).exists()) {
        ...
        star_index_ch = Channel.fromPath(params.star_genome_index, checkIfExists: true)

    } else if (file("${params.ref_base}/reference/STAR_index").exists()) {
        println " Reusing STAR index from: ${params.ref_base}/reference/STAR_index"
        star_index_ch = Channel.fromPath("${params.ref_base}/reference/STAR_index", checkIfExists: true)
